...
function SortableRow(props: RowProps) {
  ...
  return (
    ...
    >
      ...
      {q.source === 'page' && !isEditing && <PageBadge />}
...
interface StarterQuestionsSectionProps {
  questions: string[]
  onChange: (questions: string[]) => void
  tier?: Tier
}

export default function StarterQuestionsSection({
  questions: externalQuestions,
  onChange,
  tier = 'enterprise',
}: StarterQuestionsSectionProps) {
  // Convert external string[] to Question[] on first render
  const [questions, setQuestionsState] = useState<Question[]>(() =>
    externalQuestions.map((text, i) => ({ id: `q-${i}-${text.slice(0, 8)}`, text, source: 'manual' as const }))
  )
  ...
  const setQuestions = (updater: Question[] | ((prev: Question[]) => Question[])) => {
    ...
    onChange(next.filter(q => q.source === 'manual').map(q => q.text))
  }
  ...
  return (
    <>
      ...
      {/* Content wrapper */}
      <div style={{ position: 'relative', minHeight: tier === 'free' ? 340 : undefined }}>
      <div style={tier === 'free' ? {
      ...
      <div style={{
        ...
        {showHowItWorks && (
          <ol style={{
            ...
            {[
              { step: '1', text: 'Set up the Webpage Awareness Action to embed your agent on a website.' },
              { step: '2', text: 'When a visitor lands on the page, CustomGPT.ai reads the page content instantly.' },
              { step: '3', text: 'CustomGPT.ai generates a question immediately relevant to what that visitor is reading and places it at the top of your starter list.' },
              { step: '4', text: 'Questions are saved for 30 days and refreshed automatically. Manage them anytime from this page.' },
            ].map((item, i, arr) => (
